...
public abstract class MessageExchanger implements AutoCloseable{
    ...
    It tries to receive a message every waitTime milliseconds.
    Parameters: totalWaitTime is the maximum amount of milliseconds that must be waited. If it's not a multiple of waitTime, it's approximated to the following multiple of waitTime.
                waitTime is the time, in milliseconds, that it must wait between each reception attempt
    Return: the received message
    Throws: MessageReceptionException if an error prevents the receiving of the message
